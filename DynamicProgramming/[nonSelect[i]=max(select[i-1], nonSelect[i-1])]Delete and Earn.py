...
class Solution:
    def deleteAndEarn(self, nums):
        ...
        for num in nums:
            mx = max(mx, num)
        total = [0] * (mx + 1) # sum_lst
        for num in nums:
            total[num] += num  # sum_lst
        first = total[0]
        second = max(total[0], total[1])
        for i in range(2, mx + 1):
            cur = max(first + total[i], second)  # cur: maxAt_i = max(maxAt_[i-2] + Sums[i-1], maxAt_[i-1])
            first = second                       # first: maxAt_(i-2)
            second = cur                         # second: maxAt_(i-1)
        return second                            # return nonSelect[mx+1]
